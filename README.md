...
Это необходимо делать каждый раз, когда вы начинаете работать с
...
``learn_django``, которую мы создали на этапе [1](#1-начало-работы)
```bash
...
Если вы видите надпись "The install worked successfuly! Congratulations!", то
...
Единственное отличие — наличие тегов Django Template Language (DTL).
Тег ``{% block content %}`` в шаблоне base.html дает 
возможность помещать в него содержимое, если данный шаблон 
...
себя информацию о пользователе, версию браузера, адрес, откуда 
...
1. view, которая будет заниматься обработкой запроса по данному пути
...
объектом реальной жизни в рассматриваемой предметной области*.
Не стоит воспринимать данное правило буквально, но следует взять его
за основу проектирования приложения для выделения сущностей предметной области,
которые будут перенесены в программный код в виде классов (моделей).
